...
namespace Api.Services
{
    public class UserService
    {
        ...
        {
            var user = await _context.Users.Include(x => x.Avatar).Include(x => x.Posts).Include(x => x.Subscribtions).Include(x => x.Followers).Include(x => x.Chats).FirstOrDefaultAsync(x => x.Id == id);
            if (user == null)
